...
export function App() {
  ...
  const [view, setView] = useState<'chat' | 'design'>('chat');
  ...
  >
    <ProjectRail projects={projects} activeId={project?.id ?? ''} onSelect={selectProject} onEdit={openProjectEditor} onRemove={target => removeProject(target.id)} onAdd={() => void openProjectPicker()} adding={pickingDirectory} />
    {sidebarOpen && <button className="panel-backdrop" aria-label="Close side panel" onClick={() => setSidebarOpen(false)} />}
    {project && <WorkspaceSidebar key={`sidebar:${project.id}`} project={project} sessions={projectSessions} activeId={activeId} onNew={newSession} onCreateFolder={createFolder} onSelect={id => { navigation.current += 1; setWorkspaceNavigation(current => ({ ...current, selectedSessions: { ...current.selectedSessions, [project.id]: id } })); setView('chat'); setSidebarOpen(false); }} onClose={() => setSidebarOpen(false)} onDesignSystem={() => { setView('design'); setSidebarOpen(false); }} onSettings={() => { setSidebarOpen(false); settings.current?.showModal(); }} onEditProject={openProjectEditor} onRemoveProject={target => removeProject(target.id)} />}
    <main className="main-panel">
      <header className="session-header"><div className="header-leading">{project && <IconButton label="Open sessions" className="mobile-nav" onClick={() => setSidebarOpen(true)}><PanelLeft /></IconButton>}<h1>{view === 'design' ? 'Design system' : session?.title ?? project?.name ?? 'KLM'}</h1>{view === 'chat' && session && <span className="mode-label">{session.status}</span>}</div><div className="header-actions">{session && view === 'chat' && <><Button size="sm" className="export-button" onClick={exportSession}>Session log<Download /></Button><IconButton label={sideOpen ? 'Close side agent' : 'Open side agent'} aria-expanded={sideOpen} onClick={() => { setSideOpen(!sideOpen); setSidebarOpen(false); }}><PanelRight /></IconButton></>}</div></header>
